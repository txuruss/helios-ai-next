...
import { updateApprovalItemStatus, bulkUpdateApprovals, bulkAssignOpsItems, assignOpsItem } from '@/lib/actions/ops'
...
export default function ApprovalQueue({ items, members, onRefresh }: Props) {
  ...
  const handleBulkAssign = () => {
    ...
    startBulk(async () => {
      ...
      capture('ops_bulk_assigned', { count: selectedIds.size, table: 'approval_items' })
      setSelectedIds(new Set()); onRefresh()
    })
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <SearchPaginationBar search={search} onSearch={(s) => { setSearch(s); setPage(1) }} page={page} totalPages={totalPages} total={filtered.length} pageSize={PAGE_SIZE} onPage={setPage} placeholder="Search approvals…" />
