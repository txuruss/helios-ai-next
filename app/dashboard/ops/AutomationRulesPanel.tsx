...
import { toggleAutomationRule, seedDefaultRules, runOpsAutomationNow } from '@/lib/actions/ops'
...
export default function AutomationRulesPanel({ rules, onRefresh }: Props) {
  ...
  const handleSeed = () => {
    ...
    startSeed(async () => {
      ...
      setSeedMsg(`Seeded ${result.seeded} default rule${result.seeded !== 1 ? 's' : ''}.`)
      onRefresh()
    })
  }
  ...
  const handleRunNow = () => {
    ...
    startRun(async () => {
      ...
      if (result.error) { setError(result.error); return }
      capture('ops_automation_run_completed', { processed: result.processed, errors: result.errors })
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-2.5 flex-wrap">
        <button
          ...
          className="h-9 px-4 rounded-[10px] text-[13px] border border-[#ff7a18]/30
                     bg-[#ff7a18]/[0.08] text-[#ffae3c] hover:bg-[#ff7a18]/15 transition-all
                     disabled:opacity-40"
        >
          {seedPending ? 'Seeding…' : '+ Seed Default Rules'}
        </button>
        <button
          onClick={handleRunNow}
          disabled={runPending}
          className="h-9 px-4 rounded-[10px] text-[13px] border border-white/[0.10]
                     bg-white/[0.04] text-[#9a9a9d] hover:bg-white/[0.08] transition-all
                     disabled:opacity-40"
        >
          {runPending ? 'Running…' : '▶ Run Automation Now'}
        </button>
        <p className="text-[11.5px] text-[#6a6a6e]">
          Processes unprocessed events against all enabled rules.
        </p>
      </div>
      ...
      {rules.length === 0 ? (
        <div className="flex flex-col items-center gap-3 py-12 text-center rounded-2xl border border-white/[0.07] bg-[#0f1012]">
          ...
          <p className="text-[12px] text-[#6a6a6e] max-w-sm">
            Click &quot;Seed Default Rules&quot; to create a set of standard rules for alerts, tasks, and approvals.
          </p>
        </div>
      ) : (
            ...
            return (
              <div key={rule.id} className="flex items-start gap-4 px-5 py-4">
                ...
                <button
                  ...
                  className={`mt-0.5 w-9 h-5 rounded-full transition-colors shrink-0 relative disabled:opacity-40
                              ${rule.is_enabled ? 'bg-[#22d093]' : 'bg-white/[0.12]'}`}
                >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className={`text-[13px] font-medium ${rule.is_enabled ? 'text-white' : 'text-[#6a6a6e]'}`}>
                      {rule.name}
                    </p>
                    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${actionCfg.color}`}>
                      {actionCfg.label}
                    </span>
                    <span className={`text-[10px] font-semibold uppercase ${PRIORITY_COLOR[rule.priority]}`}>
                      {rule.priority}
                    </span>
                  </div>
                  {rule.description && (
                    <p className="text-[11.5px] text-[#6a6a6e] mt-0.5">{rule.description}</p>
                  )}
                  <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                    {rule.trigger_source && (
                      <TriggerPill label="Source" value={rule.trigger_source} />
                    )}
                    {rule.trigger_event_type && (
                      <TriggerPill label="Event" value={rule.trigger_event_type} />
                    )}
                    {rule.trigger_severity && (
                      <TriggerPill label="Severity" value={rule.trigger_severity} />
                    )}
                    <span className="text-[10.5px] text-[#6a6a6e]">→</span>
                    <span className="text-[11px] text-white font-mono truncate">
                      {rule.action_title_template}
                    </span>
                  </div>
                </div>
                ...
                <span className={`shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full
                                  ${rule.is_enabled
                                    ? 'bg-[#22d093]/10 text-[#22d093]'
                                    : 'bg-white/[0.06] text-[#6a6a6e]'}`}>
