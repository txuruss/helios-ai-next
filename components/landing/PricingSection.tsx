      ...
      <div className="max-w-[1280px] mx-auto px-7 border-t border-white/[0.06] pt-28">
        <div className="flex flex-col gap-3.5 mb-14 max-w-[760px]">
          ...
          <h2 className="text-[clamp(28px,3.5vw,44px)] font-semibold tracking-tight leading-[1.08]">
            Choose the system that fits your business stage.
          </h2>
          <p className="text-[16px] text-[#9a9a9d]">
            Every package starts with a free AI Opportunity Audit so you know exactly what you&apos;re getting.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-stretch">
          {PRICING_TIERS.map((tier, i) => (
            ...
            >
              ...
              <div>
                <div className="text-[15px] font-semibold uppercase tracking-[0.14em] text-[#ffae3c]">
                  {tier.name.split(' ')[0]}
                </div>
              ...
              <div className="border-y border-white/[0.06] py-3.5 flex flex-col gap-2">
                {['Setup', 'Monthly support'].map((label) => (
                  <div key={label} className="flex justify-between items-center text-[13px]">
                    <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-[#6a6a6e]">{label}</span>
                    <span className="font-mono text-[#ffae3c] font-medium">Custom Quote</span>
                  </div>
                ))}
              </div>

              <p className="text-[14px] text-[#9a9a9d]">{tier.tagline}</p>
              ...
              <div className="p-3.5 rounded-xl bg-[#ff7a18]/[0.08] border border-[#ff7a18]/20 flex flex-col gap-1 mt-auto">
                <div className="text-[10.5px] font-semibold uppercase tracking-[0.14em] text-[#ffae3c]">Best outcome</div>
        ...
        <p className="text-center mt-7 text-[13px] text-[#6a6a6e]">
          Final pricing depends on business workflow, integrations, and system complexity.
        </p>
        <div className="mt-4 max-w-[640px] mx-auto flex items-start gap-2.5 p-4 rounded-xl
          ...
          You don&apos;t need to know what AI tools you need. Helios AI reviews your workflow first,
          then recommends the system that fits your business.
        </div>
      </div>
    </section>
  )
