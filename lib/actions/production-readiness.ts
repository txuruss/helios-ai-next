  ...
  try {
    ...
    const checks: ReadinessCheck[] = [
      ...
      },
      {
        ...
        status: process.env.STRIPE_SECRET_KEY ? 'configured' : 'optional',
        required: false,
        description: 'Required for billing and plan upgrades.',
        fixHint: 'Set STRIPE_SECRET_KEY. Not needed for beta if billing is manual.',
      },
      {
        key: 'stripe_webhook',
        label: 'Stripe Webhook Secret',
        status: process.env.STRIPE_WEBHOOK_SECRET ? 'configured' : 'optional',
        required: false,
        description: 'Verifies Stripe webhook events.',
        fixHint: 'Set STRIPE_WEBHOOK_SECRET after configuring the Stripe webhook endpoint.',
      },
