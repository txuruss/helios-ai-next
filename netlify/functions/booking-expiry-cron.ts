...
export default async function bookingExpiry() {
  ...
  try {
    const response = await fetch(`${appUrl}/api/cron/bookings/expire-confirmations`, {
      method:  'POST',
      headers: {
    ...
    console.log(`[netlify/booking-expiry-cron] ${response.status}`)

    if (!response.ok) {
      const text = await response.text().catch(() => '')
      console.error(`[netlify/booking-expiry-cron] Error ${response.status}: ${text.slice(0, 120)}`)
    ...
    console.error('[netlify/booking-expiry-cron] Fetch error:', msg)
...
export const config: NetlifyConfig = { schedule: '*/15 * * * *' }
